...
def parse_args(
    ...
    if args.exp_group_name is not None:
        ...
        args.checkpoints_path = os.path.join(
            args.checkpoints_path, args.exp_group_name, str(args.stage)
        )
        args.visual_path = os.path.join(args.visual_path, args.exp_group_name)

    os.makedirs(
        os.path.join(args.checkpoints_path, args.name, str(args.stage)), exist_ok=True
    )
    os.makedirs(os.path.join(args.visual_path, args.name), exist_ok=True)

    PROJECT_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
    # EXPCONF_PATH = os.path.join(PROJECT_ROOT, "conf", "RRF.conf")
    ...
    if args.enable_refr:
        print("* ENABLE REFRACTION")
    else:
        print("* DISABLE REFRACTION")

    if args.enable_refl:
        print("* ENABLE REFLECTION")
    else:
