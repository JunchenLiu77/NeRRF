...
from model.grad_sample import grid_sample
from IPython import embed

def grid_sample_old(image, optical):
...
class SpatialEncoder(nn.Module):
    ...
    def index(self, uv, cam_z=None, image_size=(), z_bounds=None):
        ...
        with profiler.record_function("encoder_index"):
            ...
            samples = grid_sample(self.latent,uv)
    ...
    def forward(self, x, depth_x=None):
        """
        ...
        ####depth feature ?######
        if self.depth and (depth_x is not None):
            depth_latent = self.depth_encoder(depth_x)
            cat_latent = torch.cat([self.latent,depth_latent],dim=1)
            self.latent = self.latent_fusion(cat_latent)
