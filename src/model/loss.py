...
def get_depth_loss(pred_depth,gt_depth,reduction="mean"):
    depth_loss = torch.nn.L1Loss(reduction=reduction)
    return depth_loss(pred_depth,gt_depth)

def get_weight_loss(weights):
    bceloss = torch.nn.BCELoss()
    weights_sum = weights.sum(dim=-1)
    return bceloss(weights_sum, torch.ones_like(weights_sum).to(weights.device))

def get_normal_loss(pred_normal,gt_normal,diff_norm=None,reduction="none"):
    diff_norm_loss = .0
    if diff_norm is not None:
        diff_norm_loss = diff_norm.mean()
    norm = pred_normal.norm(2, dim=-1).unsqueeze(-1) 
    pred_mask = norm > 0.5
    normal_loss = torch.nn.MSELoss(reduction=reduction)
    normal_loss_ = normal_loss(pred_normal,gt_normal) * pred_mask.repeat(1,1,3)
    return normal_loss_.mean() + diff_norm_loss

def MAE(pred,gt):
    return torch.mean(abs(gt - pred))
