...
def save_uint16_png(path, image):
    ...
    ), "data type of the array should be np.uint16." + "Got {}".format(image.dtype)
    assert (
        len(image.shape) == 2
    ), "Shape of input image should be (H, W)" + "Got {}".format(len(image.shape))
...
def _normalize_depth_img(depth_img, dtype=np.uint8, min_depth=0.0, max_depth=1.0):
    ...
    if dtype != np.uint16 and dtype != np.uint8:
        raise ValueError(
            'Unsupported dtype {}. Must be one of ("np.uint8", "np.uint16")'.format(
                dtype
            )
        )
    ...
    depth_img = np.ma.filled(
        depth_img, fill_value=0
...
def depth2rgb(
    ...
    if dynamic_scaling:
        ...
        depth_img_scaled = _normalize_depth_img(
            depth_img, dtype=np.uint8, min_depth=min_depth, max_depth=max_depth
        )

    if reverse_scale is True:
        depth_img_scaled = np.ma.masked_array(
            depth_img_scaled, mask=(depth_img_scaled == 0.0)
        )
...
def scale_depth(depth_image):
    ...
    ), "data type of the array should be float32. Got {}".format(depth_image.dtype)
    SCALING_FACTOR = 4000
...
def unscale_depth(depth_image):
    ...
    assert (
        depth_image.dtype == np.uint16
    ), "data type of the array should be uint16. Got {}".format(depth_image.dtype)
    SCALING_FACTOR = 4000
...
def normal_to_rgb(normals_to_convert, output_dtype="float"):
    ...
    if output_dtype == "uint8":
        ...
        raise NotImplementedError(
            'Possible values for "output_dtype" are only float and uint8. received value {}'.format(
                output_dtype
            )
        )
...
def _get_point_cloud(color_image, depth_image, fx, fy, cx, cy):
    ...
    camera_points_x = np.multiply(
        pixel_x - camera_intrinsics[0, 2], (depth_image / camera_intrinsics[0, 0])
    )
    camera_points_y = np.multiply(
        pixel_y - camera_intrinsics[1, 2], (depth_image / camera_intrinsics[1, 1])
    )
    camera_points_z = depth_image
    camera_points = (
        np.array([camera_points_x, camera_points_y, camera_points_z])
        .transpose(1, 2, 0)
        .reshape(-1, 3)
    )
    ...
    with open(filename, "wb") as fid:
        ...
        for i in range(xyz_points.shape[0]):
            fid.write(
                bytearray(
                    struct.pack(
                        "fff", xyz_points[i, 0], xyz_points[i, 1], xyz_points[i, 2]
                    )
                )
            )  # ,
