...
class EnvironmentLight(torch.nn.Module):
    ...
    def build_mips(self, cutoff=0.99):
        self.specular = [self.base.clone().detach()]
    ...
    def shade(self, gb_pos, gb_normal, kd, ks, view_pos, specular=True):
        ...
        if self.mtx is not None:  # Rotate lookup
            ...
            reflvec = ru.xfm_vectors(
                reflvec.view(
                    reflvec.shape[0],
                    reflvec.shape[1] * reflvec.shape[2],
                    reflvec.shape[3],
                ),
                mtx,
            ).view(*reflvec.shape)
            nrmvec = ru.xfm_vectors(
                nrmvec.view(
                    nrmvec.shape[0], nrmvec.shape[1] * nrmvec.shape[2], nrmvec.shape[3]
                ),
                mtx,
            ).view(*nrmvec.shape)
        ...
        if specular:
            ...
            if not hasattr(self, "_FG_LUT"):
                self._FG_LUT = torch.as_tensor(
                    np.fromfile("src/bsdf_256_256.bin", dtype=np.float32).reshape(
                        1, 256, 256, 2
                    ),
                    dtype=torch.float32,
                    device="cuda",
            ...
            fg_lookup = dr.texture(
                self._FG_LUT, fg_uv, filter_mode="linear", boundary_mode="clamp"
            )
