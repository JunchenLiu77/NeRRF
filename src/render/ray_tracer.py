...
class RayTracing(nn.Module):
    def RayTracer(
        self,
        object_bounding_sphere=1.0,  # todo stage 1 parameter
        sdf_threshold=5.0e-5,
        line_search_step=0.5,  # 回退步长
        line_step_iters=1,  # 回退至多一次
        sphere_tracing_iters=10,
        n_steps=100,
        n_secant_steps=8,  # todo
    ):
        super().__init__()
    ...
    def forward(self, sdf, cam_loc, object_mask, ray_directions):
        ...
        # 没有hit的start>=end，network包括hit的和不收敛的
        ...
        print("----------------------------------------------------------------")
        print(
            "RayTracing: object = {0}/{1}, secant on {2}/{3}.".format(
                network_object_mask.sum(),
                len(network_object_mask),
                sampler_net_obj_mask.sum(),
                sampler_mask.sum(),
            )
        )
        print("----------------------------------------------------------------")
    ...
    def sphere_tracing(
        ...
        sphere_intersections_points = cam_loc.reshape(
            batch_size, 1, 1, 3
        ) + sphere_intersections.unsqueeze(-1) * ray_directions.unsqueeze(
            2
        )  # sphere上的相交点
        ...
        curr_start_points[unfinished_mask_start] = sphere_intersections_points[
            ...
            unfinished_mask_start
        ]  # 和球相交的初始点为相交点，否则为0（todo 感觉应该是cam_loc？）
        ...
        next_sdf_start = torch.zeros_like(acc_start_dis).cuda()  # sdf，不相交的点为0
        ...
        while True:
            ...
            curr_sdf_start[curr_sdf_start <= self.sdf_threshold] = 0  # sdf小于阈值的点sdf=0
            ...
            while (
                ...
                acc_start_dis[not_projected_start] -= (
                    (1 - self.line_search_step) / (2**not_proj_iters)  # 深度回退，比较少见
            ...
            # 无法保证start和end point的sdf值为正，但大概率是

            # 穿过球但没hit到物体，acc dis start<end
            unfinished_mask_start = unfinished_mask_start & (
    ...
    # 处理不收敛的
    # todo object mask或许可以从mask图拿到，但ray格式不一样
    def ray_sampler(
        self, sdf, cam_loc, object_mask, ray_directions, sampler_min_max, sampler_mask
    ):
        ...
        points = cam_loc.reshape(batch_size, 1, 1, 3) + pts_intervals.unsqueeze(
            -1
    ...
    def minimal_sdf_points(
        self, num_pixels, sdf, cam_loc, ray_directions, mask, min_dis, max_dis
    ):
